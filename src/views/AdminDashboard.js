...
export default class AdminDashboard extends React.Component {
  ...
  componentWillMount() {
    // get user if it doesn't already exist
    if (!this.state.currentUser || this.state.currentUser === -1) {
      // CHECK LOGIN
  ...
  handleNewArtwork = () => {
    ...
    if (!this.isValidUrl(this.state.urlToAdd))
      this.setState({
        newArtworkErrorMessage: "Invalid URL, please try again"
      });
    else this.props.history.push("/admin-edit-artwork/" + this.state.urlToAdd);
  };
  ...
  render() {
          ...
          <View style={{ flex: 1, alignContent: "center" }}>
            ...
            <TextInput
              ...
              onChangeText={urlToAdd => {
                this.setState({
                  urlToAdd,
                  newArtworkErrorMessage: this.isValidUrl(urlToAdd)
                    ? null
    ...
    if (currentUser !== -1) {
      ...
      if (currentUser === null) this.props.history.push("/admin");
      else viewToRender = DashBoard;
    }
